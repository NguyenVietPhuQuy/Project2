...
function searchProduct(){
    ...
    {let productCompare = product_list[i].type.toLowerCase()
    if (productCompare.includes(keyword)){search.push(product_list[i])}
    };
    //check id
    for ( let i=0;i<product_list.length;i++)
    {let productId = product_list[i].id.toLowerCase()
    if (productId.includes(keyword)){search.push(product_list[+i+1])}
    };
    //check name
    for ( let i=0;i<product_list.length;i++)
    {let productName = product_list[i].name.toLowerCase()
    if (productName.includes(keyword)){search.push(product_list[i])}
    };
    //check screen
    for ( let i=0;i<product_list.length;i++)
    {let productScreen = product_list[i].screen.toLowerCase()

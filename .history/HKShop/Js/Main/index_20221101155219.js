...
let filterProduct = ()=>{
    product__list.fetchApi()
    .then((response)=>{
        let filterValue = document.getElementById('search__select').value;
        let result = [];
        result =response.data;
        if(filterValue='') product__list = response.data;
        else {
            result.map((productResult)=>{if(productResult.type === filterValue) {result.push(productResult)}})
        }
        renderData(productResult)

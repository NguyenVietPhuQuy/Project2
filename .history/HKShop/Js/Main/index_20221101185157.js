...
/**4- Function về Cart  **/
  ...
  // tạo ra 1 biến để lưu thông tin mảng trả về local storage
    const dataSaving = JSON.parse(localStorage.getItem("cart"))  || [];
    function updateData(){
        localStorage.setItem("cart",JSON.stringify(dataSaving));
    }
    updateData() 
  ...
  // tạo ra hàm onclick trên cart mỗi khi nhấn
    let addToCart =(id)=>{
        if(dataSaving.some(item=>item.id === id)){
            if(dataSaving.some(item=>item.id ===id)){item.quantity++}}
        else {

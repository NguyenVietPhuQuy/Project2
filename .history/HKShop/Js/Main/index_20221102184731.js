    ...
    dataSaving.forEach(item => {
        content += `
         <tr>
            <td>
                <div>
                    <img style="width: 70px;height:90px"src=${item.img}alt="">
                    <div class=name>${item.name}</div>
                </div>
            </td>
                   <button type="button"  onclick="changeQuatityPlus('${item.id}')">+</button>
                   <div class="number">${item.quantity}</div>
                   <button type="button" onclick="changeQuatityMinus('${item.id}')">-</button>
              </td>

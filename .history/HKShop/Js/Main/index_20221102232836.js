                ...
                <div class="product__design">
                    ...
                    <div class="product__content">
                        <header class="product__desc" id="">${result.desc}</header>
                         ...
                         <div class="product__price">price:<span class="product__price-span">${result.price}</span></div>
                        <div class="product__screen">Screen:<span class="product__screen-span">${result.screen}</span></div>
                         ...
                         <div class="product__front">Front:<span class="product__front-span">${result.frontCamera}</span></div>
                        </div>
...
function renderCartItem() {
    ...
    dataSaving.forEach(item => {
        content += `
         <tr>
            <td>
                    <img style="width: 70px;border-radius:10px;"src=${item.img}
                    alt="">${item.name}
            </td>

            <td>
                <div>
                   <button type="button" style="width: 20px; border-radius:5px 5px 0 0;outline:none; border:none" onclick="changeQuatityPlus('${item.id}')">+</button>
                   <div class="number" style="width: 100%;display:flex;justify-content:center"; >${item.quantity}</div>
                   <button type="button"style="width: 20px;border-radius:0 0 5px 5px;outline:none; border:none " onclick="changeQuatityMinus('${item.id}')">-</button>
                </div>
            </td>

            <td>
                <span style="display:block;margin-top:18px;" >$${item.price}</span>
            </td>
